...
if [ -z "$project" ] || [ "$project" = "" ] || [ "$project" = " " ];
	...
	virtualenv -p python3 venv && . venv/bin/activate && venv/bin/pip3 install -r requirements.txt && pre-commit install && echo "${green}Done!${white}" || { cd ..; rm -rf "$project"; echo "${red}Exiting${white}"; exit 1; }
fi
